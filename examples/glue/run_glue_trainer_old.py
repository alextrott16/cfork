...
def spawn_finetuning_jobs(
    ...
    #             pool.apply_async(
    #                 train_finetune,
    #                 args=(base_yaml_file, task, save_ckpt, ckpt_load_path, parent_ckpt, parent_idx, ckpt_save_folder,
    #                       save_locally, free_port + rank, load_ignore_keys, seed),
    #                 callback=partial(
    #                     ingest_finetuning_result,
...
def _convert_torch_tensor_to_primitives(metrics_dict: dict):
    ...
    for k, v in metrics_dict.items():
        if isinstance(v, torchmetrics.Metric):
            metrics_dict[k] = v.compute().item()  # <-- Updating API call
...
def _main() -> None:
    ...
    if training_scheme in ('finetune', 'all'):
        ...
        results = run_finetuner(training_scheme, file, save_locally, finetune_hparams.save_folder, finetune_hparams,
                                glue_metrics)
