...
class BertSelfAttentionPre(torch.nn.Module):
    ...
    def __init__(self, module: torch.nn.Module, layer_norm_eps: float = 1e-12, head_scale: bool = False):
        super().__init__()
        assert isinstance(module, BertSelfAttention)
        self.use_head_scale = head_scale
    ...
    def _apply_head_scaling(self, attn_output):
        ...
        assert self.use_head_scale and self.head_scales is not None
    ...
    def forward(
        ...
        # The full NormFormer does Head Scaling
        if self.use_head_scale:
...
class BertIntermediatePre(torch.nn.Module):
    ...
    def __init__(self, module: torch.nn.Module, layer_norm_eps: float = 1e-12, ffn_layernorm: bool = False):
        super().__init__()
...
class BertSelfOutputPre(torch.nn.Module):
    ...
    def __init__(self, module: torch.nn.Module, layer_norm_eps: float = 1e-12, attn_output_layernorm: bool = False):
        super().__init__()
...
class BertOutputPre(torch.nn.Module):
    ...
    def __init__(self, module: torch.nn.Module, layer_norm_eps: float = 1e-12, last_layer: bool = False):
        super().__init__()
...
class BertGatedFFOutputPre(torch.nn.Module):
    ...
    Note: If the BertGatedFFOutput layer is present, it has likely replaced the BertIntermediate and BertOutput pair.
