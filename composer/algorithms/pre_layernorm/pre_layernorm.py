...
def apply_pre_layernorm(model: torch.nn.Module,
                        ...
                        normformer: Optional[bool] = False) -> None:
    """
    ...
    Args:
        ...
        optimizers (`torch.optim.Optimizer` | Sequence[`torch.optim.Optimizer`], optional):
            ...
            model parameters.
        layer_norm_eps (float, optional): The epsilon to use for newly created LayerNorm modules, default ``1e-12``.
    ...
    policy: Dict[Type[torch.nn.Module], module_surgery.ReplacementFunction] = {
        ...
        BertOutput:
            lambda module, idx: BertOutputPre(module, layer_norm_eps=layer_norm_eps, normformer=normformer),
    }
...
class PreLayerNorm(Algorithm):
    ...
    def __init__(self, layer_norm_eps: Optional[float] = 1e-12, normformer: Optional[bool] = False):
        if not IS_TRANSFORMERS_INSTALLED:
            raise MissingConditionalImportError(extra_deps_group='nlp', conda_package='transformers')
        self.layer_norm_eps = layer_norm_eps
        assert self.layer_norm_eps > 0
