...
class SeqLengthWarmup(Algorithm):
    ...
    def apply(self, event: Event, state: State, logger: Logger) -> Optional[int]:
        ...
        if not self._activated and state.grad_accum not in self._failed_grad_accums:
            ...
            print(
                f"SLW activation {'SUCCESSFUL' if self._activated else 'FAILED'}. "
        ...
        state.batch = set_batch_sequence_length(
            state.batch, curr_seq_len, self.truncate, self.preserve_eos
        )
